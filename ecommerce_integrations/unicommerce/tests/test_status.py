...
class TestUnicommerceStatusUpdates(TestCaseApiClient):
    ...
    def test_delete_cancelled_items(self):
        item1 = frappe.new_doc("Sales Order Item").update(
            {ORDER_ITEM_CODE_FIELD: "cancelled"}
        )
        item2 = frappe.new_doc("Sales Order Item").update(
            {ORDER_ITEM_CODE_FIELD: "not cancelled"}
        )
