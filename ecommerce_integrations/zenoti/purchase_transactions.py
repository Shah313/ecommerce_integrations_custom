...
def process_purchase_orders(center, error_logs, date=None):
    list_of_purchase_orders_for_center = get_list_of_purchase_orders_for_center(
        center.name, date
    )
    if list_of_purchase_orders_for_center and len(
        list_of_purchase_orders_for_center.get("orders")
    ):
...
def process_purchase_order(order, center, error_logs):
    required_data = get_required_data_to_create_purchase_record(
        order, center, error_logs
    )
    if required_data:
        for items in required_data:
            ...
            if supplier_err_msg:
                msg = (
                    _("For Order no {}.").format(items["order_number"])
                    + " "
                    + supplier_err_msg
                )
                error_logs.append(msg)

            item_err_msg_list = check_for_item(
                items["item_data"], item_group="Products"
            )
            ...
            if all_suppliers:
                for supplier in all_suppliers["vendors"]:
                    if not frappe.db.exists(
                        "Supplier", {"zenoti_supplier_code": supplier["code"]}
                    ):
                        try:
...
def get_required_data_to_create_purchase_record(order, center, error_logs):
    ...
    if not frappe.db.exists(
        ...
        "Purchase Order", {"zenoti_order_no": order["order_number"]}
    ):
        ...
        item_data, item_err_msg_list = process_purchase_partials(
            order.get("partials"), cost_center
        )
        if len(item_err_msg_list):
            item_err_msg = "\n".join(err for err in item_err_msg_list)
            msg = (
                _("For Order no {}.").format(order["order_number"])
                + "\n"
                + item_err_msg
            )
            error_logs.append(msg)
...
def create_purchase_record(order):
    ...
    doc.buying_price_list = frappe.db.get_single_value(
        "Zenoti Settings", "default_buying_price_list"
    )
    doc.set_warehouse = frappe.db.get_single_value(
        "Zenoti Settings", "default_purchase_warehouse"
    )
    doc.set("items", [])
    ...
    for item in item_data:
        ...
        for key, value in item.items():
            ...
            if key == "item_code":
                item_code = frappe.db.get_value(
                    "Item",
                    {
                        "zenoti_item_code": item["item_code"],
        ...
        if invoice_item.get("item_tax_template"):
            invoice_item["item_tax_template"] = frappe.db.get_value(
                "Item Tax Template",
                filters={"title": invoice_item["item_tax_template"]},
            )
            invoice_item["item_tax_rate"] = get_item_tax_rate(
