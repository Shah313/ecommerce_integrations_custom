...
def _create_new_customer(order):
    ...
    address = order.get("billingAddress") or (
        order.get("addresses") and order.get("addresses")[0]
    )
    address.pop(
        "id", None
    )  # this is not important and can be different for same address
...
def _check_if_customer_exists(address, customer_code):
    ...
    if customer_code:
        customer_name = frappe.db.get_value(
            "Customer", {CUSTOMER_CODE_FIELD: customer_code}
        )

    if not customer_name:
        customer_name = frappe.db.get_value(
            "Customer", {ADDRESS_JSON_FIELD: json.dumps(address)}
        )
