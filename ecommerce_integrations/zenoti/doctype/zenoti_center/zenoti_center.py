...
from ecommerce_integrations.zenoti.doctype.zenoti_settings.zenoti_settings import (
    sync_invoices,
)
...
class ZenotiCenter(Document):
            ...
            for page in range(1, total_page + 2):
                ...
                if all_customers:
                    for customer in all_customers["guests"]:
                        if not frappe.db.exists(
                            "Customer", {"zenoti_guest_id": customer["id"]}
                        ):
    ...
    def sync_category(self):
        url = (
            ...
            + "centers/"
            + str(self.name)
            + "/categories?include_sub_categories=true"
        )
            ...
            for page in range(1, total_page + 2):
                ...
                if all_categories:
                    for category in all_categories["categories"]:
                        if not frappe.db.exists(
                            "Zenoti Category", {"category_id": category["id"]}
                        ):
    ...
    def create_emp(self, emp):
        ...
        doc.zenoti_employee_username = (
            emp["personal_info"]["user_name"]
            if "user_name" in emp["personal_info"]
            else ""
        )
...
def sync(center, record_type, start_date=None, end_date=None):
    if record_type == "Sales Invoice":
        ...
        if date_diff(end_date, start_date) > 7:
            frappe.throw(
                _("Difference between From Date and To Date cannot be more than 7.")
            )
        frappe.enqueue(
