...
def get_dummy_price_list() -> str:
    ...
    if not frappe.db.exists("Price List", DUMMY_PRICE_LIST):
        pl = frappe.get_doc(
            doctype="Price List", price_list_name=DUMMY_PRICE_LIST, selling=1
        ).insert()
        pl.add_comment(
            text=_("This price list is used by integrations and should be left empty")
        )
...
def discard_item_prices(doc, method=None):
    ...
    if doc.price_list == DUMMY_PRICE_LIST:
        frappe.enqueue(
            method=_delete_all_dummy_prices, queue="short", enqueue_after_commit=True
        )
