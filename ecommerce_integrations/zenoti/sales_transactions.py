...
def process_sales_invoices(center, error_logs, start_date=None, end_date=None):
    ...
    list_of_invoice_for_center = get_list_of_invoices_for_center(
        center.name, start_date, end_date
    )
...
def get_invoice_details(invoice, center, error_logs):
    ...
    if not frappe.db.exists(
        "Sales Invoice", {"zenoti_invoice_no": invoice[0]["invoice_no"]}
    ):
        data = validate_details(invoice, center, error_logs)
        customer = frappe.db.exists(
            "Customer", {"zenoti_guest_id": invoice[0]["guest"]["guest_id"]}
        )
        if data:
...
def validate_details(invoice, center, error_logs):
    data = {}
    err_msg = check_for_customer(
        invoice[0]["guest"]["guest_id"], invoice[0]["guest"]["guest_name"]
    )
    if err_msg:
    ...
    if not cost_center:
        cost_center_err_msg = _(
            "Center {0} is not linked to any ERPNext Cost Center."
        ).format(frappe.bold(center.get("center_name")))
...
def make_employee(emp_name, emp_code):
    ...
    if not employee:
        err_msg = _("Details for Employee {0} not found in Zenoti").format(
            frappe.bold(emp_name)
        )
    return err_msg
...
def make_error_log_msg(invoice, err_msg, error_logs):
    ...
    msg = (
        _("For {0} {1}.").format(frappe.bold(invoice_no), frappe.bold(receipt_no))
        + "\n"
        + err_msg
    )
...
def process_sales_line_items(invoice, cost_center, center):
    ...
    for line_item in invoice:
        ...
        if not sold_by:
            msg = _("Employee {} not found in ERPNext.").format(
                frappe.bold(line_item["employee"]["name"])
            )
        ...
        if len(err_msg_list) == 0:
            ...
            qty = (
                line_item["quantity"]
                if line_item["sale_price"] >= 0
                else line_item["quantity"] * -1
            )
            tip += line_item["tips"]
            item = frappe.db.get_value(
                "Item", {"zenoti_item_code": line_item["item"]["code"]}, "name"
            )
            item_dict = {
                "item_code": item,
            ...
            if item_group == "Gift or Pre-paid Cards":
                item = frappe.db.get_value(
                    "Item",
                    {"zenoti_item_code": "Card No. " + line_item["item"]["code"]},
                    "name",
                )
...
def make_customer(guest_id, guest_name):
    ...
    if not guest_details:
        err_msg = _("Details for Guest {} not found in Zenoti").format(
            frappe.bold(guest_name)
        )
        return err_msg
...
def prepare_customer_details(guest_details):
    ...
    if guest_details["personal_info"]["gender"]:
        customer_details["gender"] = guest_gender_map[
            guest_details["personal_info"]["gender"]
        ]
    if guest_details["address_info"]:
        ...
        customer_details["phone"] = guest_details["personal_info"]["mobile_phone"][
            "number"
        ]
...
def check_for_items(item, center):
    ...
    if not frappe.db.exists(
        "Item",
        {"zenoti_item_code": item["item"]["code"], "item_group": item_type[group]},
    ):
        if group == 6:
            err_msg = make_card_item(item)
        else:
            item_to_search = {
                "zenoti_item_code": item["item"]["code"],
                "name": item["item"]["name"],
            }
