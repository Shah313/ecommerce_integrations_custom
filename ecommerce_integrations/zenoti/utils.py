...
def make_api_call(url):
    ...
    if res_headers.get("RateLimit-Reset"):
        frappe.flags.zenoti_rate_limit_reset_time = cint(
            res_headers.get("RateLimit-Reset")
        )
...
def get_headers():
    headers = {}
    headers["Authorization"] = "apikey " + frappe.db.get_single_value(
        "Zenoti Settings", "api_key"
    )
    return headers
...
def create_item(item_dict, item_details, item_group, center):
    ...
    item.zenoti_item_code = (
        item_details["code"] if "code" in item_details else item_dict["code"]
    )
    ...
    if item_details.get("category_id"):
        item.zenoti_item_category = get_zenoti_category(
            item_details.get("category_id"), center
        )
    if item_details.get("sub_category_id"):
        item.zenoti_item_sub_category = get_zenoti_category(
            item_details.get("sub_category_id"), center
        )
...
def get_zenoti_category(category_id, center):
    ...
    return frappe.db.get_value(
        "Zenoti Category", {"category_id": category}, "category_name"
    )
...
def check_for_item_tax_template(item_tax_template):
    err_msg = ""
    if item_tax_template and not frappe.db.exists(
        "Item Tax Template", item_tax_template
    ):
        err_msg = _("Item Tax Template {} does not exist.").format(
            frappe.bold(item_tax_template)
        )
    return err_msg
