...
from ecommerce_integrations.zenoti.stock_reconciliation import (
    process_stock_reconciliation,
)
...
def sync_invoices(center_id=None, start_date=None, end_date=None):
    if cint(frappe.db.get_single_value("Zenoti Settings", "enable_zenoti")):
        if center_id or cint(
            frappe.db.get_single_value("Zenoti Settings", "enable_auto_syncing")
        ):
            check_perpetual_inventory_disabled()
            ...
            for row in list_of_centers:
                ...
                if (
                    last_sync
                    and get_datetime()
                    > get_datetime(add_to_date(last_sync, hours=cint(interval)))
                ) or (start_date and end_date):
                    error_logs = []
                    process_sales_invoices(center, error_logs, start_date, end_date)
                    if not last_sync or get_datetime(last_sync) < get_datetime(
                        end_date
                    ):
