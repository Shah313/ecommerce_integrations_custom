...
class UnicommerceShipmentManifest(Document):
    ...
    def set_unicommerce_details(self):
        ...
        for package in self.manifest_items:
            ...
            if self.channel_id != package_info.get(CHANNEL_ID_FIELD):
                frappe.throw(
                    _(
                        "Row #{} : Only {} channel packages can be added in this manifest"
                    ).format(package.idx, self.channel_id)
                )

            if cint(package_info.get(MANIFEST_GENERATED_CHECK)):
                frappe.throw(
                    _(
                        "Row #{}: Manifest is already generated, please remove package."
                    ).format(package.idx)
                )
    ...
    def get_facility_code(self) -> str:
        ...
        if len(facility_codes) != 1:
            frappe.throw(
                _(
                    "Shipping manifest should only have one facility code, found: {}"
                ).format(",".join(facility_codes))
            )
    ...
    def update_manifest_status(self):
        ...
        frappe.db.set_value(
            "Sales Invoice", {"name": ("in", si_codes)}, MANIFEST_GENERATED_CHECK, 1
        )
...
@frappe.whitelist()
def search_packages(
    search_term: str, channel: str | None = None, shipper: str | None = None
):
    filters = {
