...
def is_unicommerce_grn(stock_entry) -> bool:
    ...
    grn_enabled = frappe.db.get_single_value(
        SETTINGS_DOCTYPE, "use_stock_entry_for_grn"
    )
    if not grn_enabled:
        frappe.throw(
            _(
                "Auto GRN not enabled in Unicommerce settings. Can not use Stock Entry Type: {}"
            ).format(GRN_STOCK_ENTRY_TYPE)
        )
    return True
...
def get_facility_code(stock_entry, unicommerce_settings) -> str:
    ...
    if len(target_warehouses) > 1:
        frappe.throw(
            _("{} only supports one target warehouse (unicommerce facility)").format(
                GRN_STOCK_ENTRY_TYPE
            )
        )

    warehouse = next(iter(target_warehouses))
    warehouse_mapping = unicommerce_settings.get_erpnext_to_integration_wh_mapping(
        all_wh=True
    )
    ...
    if not facility:
        msg = _(
            "{} warehouse does not have Unicommerce facilities mapped to it."
        ).format(warehouse)
...
def upload_grn(doc, method=None):
    ...
    if response.successful and not errors:
        ...
        frappe.msgprint(
            "Partial success, unicommerce reported errors:<br>{}".format(
                "<br>".join(errors)
            )
        )
...
def _prepare_grn_import_csv(stock_entry) -> str:
    ...
    for item in stock_entry.items:
        ...
        if not sku:
            frappe.throw(
                _("Item {} does not have associated Unicommerce SKU.").format(
                    item.item_code
                )
            )
