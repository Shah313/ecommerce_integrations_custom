    ...
    for fulfillment in shopify_order.get("fulfillments"):
        if (
            ...
            try:
                ...
                frappe.logger().info(
                    f"[Shopify] Created Sales Invoice for Order {shopify_order.get('id')}"
                )
            except Exception as e:
                frappe.logger().error(
                    f"[Shopify] Failed to create Sales Invoice for Order {shopify_order.get('id')}: {e}"
                )
