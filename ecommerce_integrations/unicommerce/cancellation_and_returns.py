...
def fully_cancel_orders(unicommerce_order_codes: list[str]) -> None:
    ...
    for order in current_orders_status:
        ...
        linked_sales_invoice = frappe.db.get_value(
            "Sales Invoice",
            filters={ORDER_CODE_FIELD: order.get(ORDER_CODE_FIELD), "docstatus": 1},
        )
...
def update_erpnext_order_items(so_data, so=None):
    ...
    cancelled_items = [
        d["code"] for d in so_data["saleOrderItems"] if d["statusCode"] == "CANCELLED"
    ]
    ...
    if not so:
        so_name = frappe.db.get_value(
            "Sales Order", {ORDER_CODE_FIELD: so_data["code"]}
        )
        if not so_name:
            return
...
def _delete_cancelled_items(erpnext_items, cancelled_items):
    items = [
        d.as_dict()
        for d in erpnext_items
        if d.get(ORDER_ITEM_CODE_FIELD) not in cancelled_items
    ]
...
def create_rto_return(package_info, client: UnicommerceAPIClient):
    ...
    rto_returns = [
        r
        for r in so_data["returns"]
        if r["type"] == "Courier Returned" and r["code"] == package_code
    ]
    if rto_returns:
...
def get_return_warehouse(facility_code):
    return frappe.db.get_value(
        ...
        {"unicommerce_facility_code": facility_code},
        "return_warehouse",
    )
...
def check_and_update_customer_initiated_returns(
    orders, client: UnicommerceAPIClient
) -> None:
...
def sync_customer_initiated_returns(so_data):
    customer_returns = [
        r for r in so_data.get("returns", []) if r["type"] == "Customer Returned"
    ]
    ...
    for customer_return in customer_returns:
        if not frappe.db.exists(
            "Sales Invoice", {RETURN_CODE_FIELD: customer_return["code"]}
        ):
...
def create_cir_credit_note(so_data, return_data):
    ...
    returned_so_codes = [
        item.get("saleOrderItemCode") for item in return_data.get("returnItems")
    ]
    returned_si_items = [
        so_si_item_map.get(so_item_code_map.get(code)) for code in returned_so_codes
    ]
    ...
    for tax in credit_note.taxes:
        ...
        for item_code, tax_distribution in item_wise_tax_detail.items():
            ...
            if not tax_distribution[1]:
                # Ignore 0 values
                continue
            return_percent = returned_qty_map.get(
                item_code, 0.0
            ) / item_code_to_qty_map.get(item_code)
