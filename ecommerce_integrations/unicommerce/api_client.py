...
class UnicommerceAPIClient:
    ...
    def request(
        ...
        if method == "GET" and "application/json" not in response.headers.get(
            "content-type"
        ):
    ...
    def create_update_item(
        self, item_dict: JsonDict, update=False
    ) -> tuple[JsonDict, bool]:
    ...
    def create_invoice_by_shipping_code(
        self, shipping_package_code: str, facility_code: str
    ):
    ...
    def create_invoice_and_assign_shipper(
        self, shipping_package_code: str, facility_code: str
    ):
        """
