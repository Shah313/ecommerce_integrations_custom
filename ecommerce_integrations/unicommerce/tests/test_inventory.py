...
class TestUnicommerceProduct(TestCaseApiClient):
    ...
    def test_inventory_sync(self):
        ...
        make_stock_entry(
            item_code="_TestInventoryItemA", qty=10, to_warehouse="Stores - WP", rate=10
        )
        make_stock_entry(
            item_code="_TestInventoryItemB", qty=2, to_warehouse="Stores - WP", rate=10
        )
        make_stock_entry(
            item_code="_TestInventoryItemC",
            qty=42,
            to_warehouse="Work In Progress - WP",
            rate=10,
        )
            ...
            "inventoryAdjustments": [
                {
                    ...
                    "quantity": get_stock_balance(
                        "_TestInventoryItemC", "Work In Progress - WP"
                    ),
                    "shelfCode": "DEFAULT",
                    "inventoryType": "GOOD_INVENTORY",
                    "adjustmentType": "REPLACE",
