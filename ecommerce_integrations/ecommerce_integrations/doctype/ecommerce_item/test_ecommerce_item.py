...
class TestEcommerceItem(unittest.TestCase):
    ...
    def test_is_synced_variant(self):
        ...
        self.assertFalse(
            ecommerce_item.is_synced("shopify", "T-SHIRT", "Unknown variant")
        )

    def test_is_synced_sku(self):
        self._create_doc_with_sku()
        self.assertTrue(
            ecommerce_item.is_synced("shopify", "T-SHIRT", sku="TEST_ITEM_1")
        )
        self.assertFalse(
            ecommerce_item.is_synced("shopify", "T-SHIRTX", sku="UNKNOWNSKU")
        )
