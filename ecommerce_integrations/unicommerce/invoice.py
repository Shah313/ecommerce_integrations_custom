...
def bulk_generate_invoices(
    ...
    for so_code in sales_orders:
        try:
            ...
            wh_allocation = (
                warehouse_allocation.get(so_code) if warehouse_allocation else None
            )
            _generate_invoice(
                client, so, channel_config, warehouse_allocation=wh_allocation
            )
        except Exception as e:
            create_unicommerce_log(
                status="Failure", exception=e, rollback=True, make_new=True
            )
...
def _log_invoice_generation(sales_orders, failed_orders):
    ...
    status = {0.0: "Failure", 100.0: "Success"}.get(
        percent_success
    ) or "Partial Success"
...
def _get_orders_with_missing_invoice(sales_orders):
    ...
    for order in sales_orders:
        ...
        invoice_exists = frappe.db.exists(
            "Sales Invoice", {ORDER_CODE_FIELD: uni_so_code}
        )
    ...
    for order, item_details in warehouse_allocation.items():
        ...
        for item_code, total_qty in item_wise_qty.items():
            ...
            if abs(total_qty - expected_qty) > 0.1:
                msg = _(
                    "Mismatch in quantity for order {}, item {} exepcted {} qty, received {}"
                ).format(order, item_code, expected_qty, total_qty)
                frappe.throw(msg)
...
def _generate_invoice(
    ...
    shipping_packages = [
        d["code"] for d in so_data["shippingPackages"] if d["status"] == "CREATED"
    ]
...
def create_sales_invoice(
    ...
    if so_data:
        ...
        if fully_cancelled:
            create_unicommerce_log(
                status="Invalid", message="Sales order was cancelled before invoicing."
            )
            return
    ...
    existing_si = frappe.db.get_value(
        "Sales Invoice", {INVOICE_CODE_FIELD: si_data["code"]}
    )
    if existing_si:
        si = frappe.get_doc("Sales Invoice", existing_si)
        create_unicommerce_log(
            status="Invalid", message="Sales Invoice already exists, skipped"
        )
        return si
    ...
    warehouse = settings.get_integration_to_erpnext_wh_mapping(all_wh=True).get(
        facility_code
    )
    ...
    si.naming_series = (
        channel_config.sales_invoice_series or settings.sales_invoice_series
    )
...
def _assign_wh_and_so_row(
    line_items, warehouse_allocation: list[ItemWHAlloc], so_code: str
):
...
def _verify_total(si, si_data) -> None:
    ...
    if abs(si.grand_total - flt(si_data["total"])) > 0.5:
        si.add_comment(
            text=f"Invoice totals mismatch: Unicommerce reported total of {si_data['total']}"
        )
...
def on_submit(self, method=None):
    ...
    unicommerce_order_code = frappe.db.get_value(
        "Sales Order", sales_order, "unicommerce_order_code"
    )
...
def on_cancel(self, method=None):
    ...
    results = frappe.db.get_all(
        "Pick List Sales Order Details",
        filters={"sales_invoice": self.name, "docstatus": 1},
    )
    if results:
