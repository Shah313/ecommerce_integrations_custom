...
class UnicommerceSettings(SettingController):
        ...
        if not self.access_token or now_datetime() >= get_datetime(self.expires_on):
            try:
                ...
                create_unicommerce_log(
                    status="Error",
                    message="Failed to authenticate with Unicommerce",
                    exception=e,
                )
    ...
    def renew_tokens(self, save=True):
        if now_datetime() >= get_datetime(self.expires_on):
            try:
                ...
                create_unicommerce_log(
                    status="Error", message="Failed to authenticate with Unicommerce"
                )
                raise e
        if save:
    ...
    def update_tokens(self, grant_type="password"):
        ...
        if grant_type == "password":
            params.update(
                {"username": self.username, "password": self.get_password("password")}
            )
        ...
        if res.status_code == 200:
            ...
            self.expires_on = add_to_date(
                now_datetime(), seconds=int(res["expires_in"])
            )
        else:
            ...
            if error and "invalid_grant" in error:
                ...
                frappe.throw(
                    _("Unicommerce reported error: <br>{}: {}").format(
                        error, description
                    )
                )
    ...
    def validate_auto_grn_settings(self):
        ...
        if not frappe.db.exists("Stock Entry Type", GRN_STOCK_ENTRY_TYPE):
            ...
            entry_type.add_comment(
                text="Entry type used for Auto GRN on unicommerce, do not modify."
            )

    def validate_warehouse_mapping(self):
        erpnext_whs = {wh_map.erpnext_warehouse for wh_map in self.warehouse_mapping}
        integration_whs = {
            wh_map.unicommerce_facility_code for wh_map in self.warehouse_mapping
        }

        if len(erpnext_whs) != len(integration_whs):
            frappe.throw(
                _(
                    "Warehouse Mapping should be unique and one-to-one without repeating same warehouses."
                )
            )
