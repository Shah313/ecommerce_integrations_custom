...
class TestUnicommerceInvoice(TestCaseApiClient):
    ...
    def test_create_invoice(self):
        ...
        si = create_sales_invoice(
            si_data=si_data, so_code=so.name, shipping_label=label
        )
        ...
        self.assertEqual(
            si.get(SHIPPING_PACKAGE_CODE_FIELD), si_data["shippingPackageCode"]
        )
        ...
        self.assertGreaterEqual(
            len(attachments), 2, msg=f"Expected 2 attachments, found: {attachments!s}"
        )
    ...
    def test_end_to_end_invoice_generation(self):
        ...
        self.responses.add(
            ...
            match=[
                responses.json_params_matcher(
                    {"shippingPackageCode": "TEST00949", "return": False}
                )
            ],
        )
        self.responses.add(
        ...
        so = create_order(order, client=self.client)
        make_stock_entry(
            item_code="MC-100", qty=15, to_warehouse="Stores - WP", rate=42
        )

        bulk_generate_invoices(sales_orders=[so.name], client=self.client)

        sales_invoice_code = frappe.db.get_value(
            "Sales Invoice", {INVOICE_CODE_FIELD: "SDU0026"}
        )
