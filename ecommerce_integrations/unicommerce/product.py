...
def import_product_from_unicommerce(
    sku: str, client: UnicommerceAPIClient = None
) -> None:
    ...
    try:
        ...
        ecommerce_item.create_ecommerce_item(
            MODULE_NAME, integration_item_code=sku, item_dict=item_dict
        )
...
def _get_item_group(category_code):
    ...
    item_group = frappe.db.get_value(
        "Item Group", {PRODUCT_CATEGORY_FIELD: category_code}
    )
    if category_code and item_group:
        return item_group

    default_item_group = frappe.db.get_single_value(
...
def upload_new_items(force=False) -> None:
    ...
    log = create_unicommerce_log(
        status="Queued", message="Item sync initiated", make_new=True
    )
...
def _build_unicommerce_item(item_code: ItemCode) -> JsonDict:
    ...
    if item_json.get("description"):
        item_json["description"] = (
            to_markdown(item_json["description"]) or item_json["description"]
        )
...
def validate_item(doc, method=None):
    ...
    if not item_group.get(PRODUCT_CATEGORY_FIELD):
        frappe.throw(
            _("Unicommerce Product category required in Item Group: {}").format(
                item_group.name
            )
        )
