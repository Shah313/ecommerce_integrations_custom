...
def prepare_delivery_note():
    try:
        ...
        # find all Facilities
        enabled_facilities = list(
            settings.get_integration_to_erpnext_wh_mapping().keys()
        )
        ...
        for facility in enabled_facilities:
            ...
            if not valid_packages:
                continue
            shipped_packages = [
                p for p in valid_packages if p["status"] in ["DISPATCHED"]
            ]
            for order in shipped_packages:
                if not frappe.db.exists(
                    "Delivery Note", {"unicommerce_shipment_id": order["code"]}, "name"
                ) and frappe.db.exists(
                    "Sales Order", {ORDER_CODE_FIELD: order["saleOrderCode"]}
                ):
                    ...
                    if frappe.db.exists(
                        "Sales Invoice",
                        {"unicommerce_order_code": sales_order.unicommerce_order_code},
                    ):
