...
def upload_erpnext_item(doc, method=None):
    """Hook: push new/updated Items to Shopify (guarded by the Shopify checkbox).
    If Shopify is unchecked, delete the linked product from Shopify.
    """
    template_item = item = doc  # alias for readability

    # ignore items created by the connector itself
    if getattr(item, "flags", None) and item.flags.from_integration:
        return
    ...
    # --- NEW: require explicit opt-in via the custom checkbox on Item ---
    def _wants_shopify_sync(itm) -> bool:
        return bool(getattr(itm, "custom_shopify", False))

    wants_sync = _wants_shopify_sync(item)

    # If this is a variant, optionally allow the template's checkbox to control it
    if not wants_sync and item.variant_of:
        try:
            tmpl = frappe.get_cached_doc("Item", item.variant_of)
            wants_sync = _wants_shopify_sync(tmpl)
        except Exception:
            pass
    # --------------------------------------------------------------------

    # Find existing Shopify link
    ...
    # --- NEW: handle deletion if unchecked ---
    if not wants_sync:
        if product_id:
            try:
                product = Product.find(product_id)
                if product:
                    product.destroy()  # delete from Shopify
                    frappe.db.delete(
                        "Ecommerce Item",
                        {
                    ...
                    frappe.logger().info(
                        f"[Shopify] Deleted product {product_id} for Item {item.name} (unchecked)."
                    )
            except Exception as e:
                frappe.logger().error(
                    f"[Shopify] Failed to delete product {product_id} for Item {item.name}: {e}"
                )
        else:
            frappe.logger().info(
                f"[Shopify] Skipped delete for {item.name}: no linked product."
            )
        return
    # --------------------------------------------------------------------

    # Connector does not upload template Items
    ...
    if is_new_product:
        ...
        if is_successful:
            ...
            product.save()  # push variant (if any)
