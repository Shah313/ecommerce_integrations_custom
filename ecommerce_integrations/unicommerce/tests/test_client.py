...
class TestCaseApiClient(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.client = UnicommerceAPIClient(
            "https://demostaging.unicommerce.com", "AUTH_TOKEN"
        )
...
class TestUnicommerceClient(TestCaseApiClient):
    ...
    def test_get_missing_item(self):
        ...
        log = frappe.get_last_doc(
            "Ecommerce Integration Log", filters={"integration": "unicommerce"}
        )
        self.assertTrue(
            "MISSING" in log.response_data, "Logging for missing item not working"
        )
    ...
    def test_get_sales_invoice(self):
        self.responses.add(
            ...
            match=[
                responses.json_params_matcher(
                    {"shippingPackageCode": "PACKAGE_ID", "return": False}
                )
            ],
        )
        ...
        self.responses.add(
            ...
            match=[
                responses.json_params_matcher(
                    {"shippingPackageCode": "PACKAGE_ID_RETURN", "return": True}
                )
            ],
        )
    ...
    def test_bulk_import(self):
        ...
        self.responses.add(
            ...
            match=[
                query_param_matcher(
                    {"name": "Auto GRN Items", "importOption": "CREATE_NEW"}
                ),
            ],
            json={"successful": True},
        )
