...
def sync_new_orders(client: UnicommerceAPIClient = None, force=False):
    ...
    if not force and not need_to_run(
        SETTINGS_DOCTYPE, "order_sync_frequency", "last_order_sync"
    ):
        return
...
def _get_new_orders(
    client: UnicommerceAPIClient, status: str | None
    ...
    configured_channels = {
        ...
        for c in frappe.get_all(
            "Unicommerce Channel", filters={"enabled": 1}, fields="channel_id"
        )
    }
...
def _create_sales_invoices(
    unicommerce_order, sales_order, client: UnicommerceAPIClient
):
    ...
    for package in shipping_packages:
        try:
            ...
            create_unicommerce_log(
                status="Error", exception=e, rollback=True, request_data=invoice_data
            )
...
def _sync_order_items(
    order: UnicommerceOrder, client: UnicommerceAPIClient
) -> set[str]:
    ...
    for item in items:
        if ecommerce_item.is_synced(
            integration=MODULE_NAME, integration_item_code=item
        ):
            continue
        else:
    ...
    so = frappe.get_doc(
        {
            ...
            "naming_series": channel_config.sales_order_series
            or settings.sales_order_series,
            ...
            "items": _get_line_items(
                ...
                default_warehouse=channel_config.warehouse,
...
def _update_package_info_on_unicommerce(so_code):
    try:
        ...
        if not shipping_packages:
            frappe.throw(
                frappe._(
                    "Shipping package not present on Unicommerce for order {}"
                ).format(so.name)
            )
        ...
        if not status:
            error_message = (
                "Unicommerce integration: Could not update package size\n"
                + json.dumps(response.get("errors"), indent=4)
            )
