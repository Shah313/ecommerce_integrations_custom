...
def update_sales_order_status():
    ...
    valid_orders = [
        order for order in updated_orders if order.get("channel") in enabled_channels
    ]
    if valid_orders:
        _update_order_status_fields(valid_orders)

    fully_cancelled_orders = [
        d["code"] for d in valid_orders if d["status"] == "CANCELLED"
    ]
    if fully_cancelled_orders:
        fully_cancel_orders(fully_cancelled_orders)

    probable_partial_cancels = [
        d for d in valid_orders if d["status"] in PARTIAL_CANCELLED_STATES
    ]
...
def _update_order_status_fields(orders):
    ...
    for order in current_orders_status:
        ...
        if old_status != new_status:
            so_code = order["name"]
            frappe.db.set_value(
                "Sales Order", so_code, ORDER_STATUS_FIELD, new_status, for_update=True
            )
...
def update_shipping_package_status():
    ...
    for facility in enabled_facilities:
        ...
        returning_packages = [
            p for p in valid_packages if p["status"] in SHIPMENT_RETURN_STATES
        ]
